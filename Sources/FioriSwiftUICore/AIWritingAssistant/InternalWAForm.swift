...
struct InternalWAForm: View {
    ...
    var body: some View {
        Group {
            ZStack {
                self.illustratedMessageView()
                    .opacity(self.context.waError == nil ? 0 : 1)
                self.listContent
                    .opacity(self.context.waError == nil ? 1 : 0)
            }
        }
        .toolbar {
        ...
        .alert("", isPresented: self.$context.showCancelAlert, actions: {
            ...
            Button {
                ...
                Text("Close".localizedFioriString())
            }
        }, message: {
            Text("Close Writing Assistant without saving changes?".localizedFioriString())
        })
    }
    ...
    @ViewBuilder func illustratedMessageView() -> some View {
        if let error = self.context.waError {
    ...
    @ViewBuilder func topLeadingButton() -> some View {
        if self.context.rewriteTextSet.count > 1, self.isTopLevel {
            ...
            if self.isTopLevel || self.context.waError != nil {
                EmptyView()
            } else {
