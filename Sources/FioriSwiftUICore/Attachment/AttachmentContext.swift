...
open class AttachmentContext: @unchecked Sendable {
    ...
    func upload(contentFrom provider: NSItemProvider) {
        ...
        self.delegate?.upload(contentFrom: provider) { [weak self] url in
    ...
    public func delete(attachment: URL) {
        ...
        if case .uploaded = configuration.attachments.first(where: { $0.primaryURL == attachment }) {
            self.delegate?.delete(url: attachment) { [weak self] url, error in
                Task { @MainActor [weak self] in
                    self?.handleDeleteCompletion(for: url, error: error, configuration: configuration)
                }
            }
    ...
    @MainActor
    private func handleDeleteCompletion(for url: URL, error: Error?, configuration: AttachmentGroupConfiguration) {
        if let error {
            if let attachmentError = error as? AttachmentError {
                switch attachmentError {
