...
struct SliderMenuItem: View {
    @Environment(\.cancelActionView) var cancelActionView
    @Environment(\.resetActionView) var resetActionView
    @Environment(\.applyActionView) var applyActionView
    ...
    var body: some View {
        SortFilterMenuItem(leftIcon: icon(name: item.icon, isVisible: true), title: item.name, rightIcon: Image(systemName: "chevron.down"), isSelected: item.isChecked)
            .onTapGesture {
...
//                )
                    ...
                    SliderPicker(value: Binding<Int?>(get: { item.workingValue }, set: { item.workingValue = $0 }), minimumValue: item.minimumValue, maximumValue: item.maximumValue)
                }
                .readHeight()
