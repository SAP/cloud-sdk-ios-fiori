...
public extension View {
    ...
    ///   - helperText: A helper text container displayed in toolbar stack view.
    ///   - moreActionOverflow: A custom overflow label for wrapped items menu.
    ///   - defaultDisplayItemsCount: Default fixed display item count.
    ///   - items: Indexed views for toolbar items
    /// - Returns: A new view with a bottom tool bar.
    func fioriToolbar<Items: IndexedViewContainer>(helperText: String? = nil,
                                                   @ViewBuilder moreActionOverflow: () -> any View = { EmptyView() },
    ...
    {
        if let text = helperText, !text.isEmpty {
            self.modifier(FioriToolbar<Items>(helperText: text, moreActionOverflow: moreActionOverflow, items: items, defaultDisplayItemsCount: defaultDisplayItemsCount))
        } else {
            self.modifier(FioriToolbar<Items>(moreActionOverflow: moreActionOverflow, items: items, defaultDisplayItemsCount: defaultDisplayItemsCount))
        }
    }
    ...
    func fioriToolbar<Items: IndexedViewContainer>(@ViewBuilder helperText: () -> any View = { EmptyView() },
                                                   ...
                                                   @IndexedViewBuilder items: () -> Items) -> some View
    {
        self.modifier(FioriToolbar<Items>(helperText: helperText, moreActionOverflow: moreActionOverflow, items: items, defaultDisplayItemsCount: defaultDisplayItemsCount))
    }
}
