...
class PopoverContext: ObservableObject {
    ...
    var presentTransition = AnyTransition.opacity.combined(with: .scale(scale: 1))
    var dismissTransition = AnyTransition.opacity.combined(with: .scale(scale: 0.001))
...
extension PopoverContext {
    func calculateFrame() -> (CGRect, UnitPoint) {
        ...
        self.presentTransition = AnyTransition.opacity.combined(with: AnyTransition.scale(scale: 1.0, anchor: animationAnchor))
        self.dismissTransition = AnyTransition.opacity.combined(with: AnyTransition.scale(scale: 0.001, anchor: animationAnchor))
