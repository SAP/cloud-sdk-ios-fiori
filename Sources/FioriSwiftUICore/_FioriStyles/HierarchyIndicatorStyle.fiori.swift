...
public struct HierarchyIndicatorBaseStyle: HierarchyIndicatorStyle {
    ...
    public func makeBody(_ configuration: HierarchyIndicatorConfiguration) -> some View {
        if configuration.isMultiline || HierarchyIndicatorConfiguration.isLargeSizeCategory(self.sizeCategory) {
            return AnyView(
...
extension HierarchyIndicatorFioriStyle {
    struct ContentFioriStyle: HierarchyIndicatorStyle {
        ...
        func makeBody(_ configuration: HierarchyIndicatorConfiguration) -> some View {
            ...
            var active = configuration.isSelected
            if let isItemActived, isMultiColumnLayout {
                active = isItemActived
            }
            return HierarchyIndicator(configuration)
                ...
                .onTapGesture {
                    if configuration.isClickable, !active, configuration.onClick != nil {
                ...
                .onChange(of: self.isActive) { oldValue, newValue in
                    if !oldValue, newValue, let onClick = configuration.onClick { // 'isActive' to true means the icon was clicked and icon was change. Now, trigger the follow-up event
                        if let itemId = onClick() { // Handle the case when indicator was in HierarchyView or HierarchyItemView
                            DispatchQueue.main.asyncAfter(deadline: .now() + 0.1) {
                                self.onCurrentItemChange?(itemId, self.columnIndex) // Handle the case when indicator was in HierarchyView only
                            }
                        }
                        DispatchQueue.main.asyncAfter(deadline: .now() + 0.1) {
                            self.isActive = false
    ...
    struct TitleFioriStyle: TitleStyle {
        ...
        func makeBody(_ configuration: TitleConfiguration) -> some View {
            ...
            Title(configuration, backgroundColorScheme: self.backgroundColorScheme, isMultiline: self.hierarchyIndicatorConfiguration.isMultiline, isLargeSizeCategory: HierarchyIndicatorConfiguration.isLargeSizeCategory(self.sizeCategory))
                .lineLimit(1)
        }
    }
    ...
    struct IconFioriStyle: IconStyle {
        ...
        func makeBody(_ configuration: IconConfiguration) -> some View {
            ...
            let icon = configuration.icon.isEmpty ? AnyView(active ? FioriIcon.actions.hierarchyOn : FioriIcon.actions.hierarchyOff) : AnyView(configuration.icon)
