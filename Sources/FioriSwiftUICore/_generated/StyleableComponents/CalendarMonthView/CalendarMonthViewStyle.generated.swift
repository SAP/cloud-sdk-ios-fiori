...
struct CalendarMonthViewDefaultStyle: CalendarMonthViewStyle {
    nonisolated init() {}

    func makeBody(_ configuration: CalendarMonthViewConfiguration) -> some View {
        CalendarMonthView(configuration)
            .calendarMonthViewStyle(CalendarMonthViewFioriStyle())
            .modifier(CalendarMonthViewStyleModifier(style: CalendarMonthViewBaseStyle()))
    }
}
