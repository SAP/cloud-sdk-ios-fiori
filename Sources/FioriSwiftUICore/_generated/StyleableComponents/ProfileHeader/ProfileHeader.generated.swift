...
public struct ProfileHeader {
    ...
    public init(@ViewBuilder detailImage: () -> any View = { EmptyView() },
                ...
                transitionProgress: Binding<CGFloat> = .constant(0),
...
public extension ProfileHeader {
    init(detailImage: Image? = nil,
         ...
         transitionProgress: Binding<CGFloat>,
         @ViewBuilder detailContent: () -> any View = { EmptyView() })
    {
        self.init(detailImage: { detailImage }, title: { Text(title) }, subtitle: { OptionalText(subtitle) }, description: { OptionalText(description) }, transitionProgress: transitionProgress, detailContent: detailContent)
    }
}
...
public extension ProfileHeader {
    ...
    internal init(_ configuration: ProfileHeaderConfiguration, shouldApplyDefaultStyle: Bool) {
        ...
        self._transitionProgress = configuration.$transitionProgress
...
extension ProfileHeader: View {
    public var body: some View {
        if self._shouldApplyDefaultStyle {
            ...
            self.style.resolve(configuration: .init(detailImage: .init(self.detailImage), title: .init(self.title), subtitle: .init(self.subtitle), description: .init(self.description), transitionProgress: self.$transitionProgress, detailContent: .init(self.detailContent))).typeErased
...
private extension ProfileHeader {
    ...
    func defaultStyle() -> some View {
        ProfileHeader(.init(detailImage: .init(self.detailImage), title: .init(self.title), subtitle: .init(self.subtitle), description: .init(self.description), transitionProgress: self.$transitionProgress, detailContent: .init(self.detailContent)))
