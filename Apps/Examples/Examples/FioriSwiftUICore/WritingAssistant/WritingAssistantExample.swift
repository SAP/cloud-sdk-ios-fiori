...
struct WritingAssistantExample: View {
    ...
    private func submitFeedback(state: AIUserFeedbackVoteState, values: [String]) async -> Bool {
    ...
    var body: some View {
        List {
            ...
            NoteFormView(text: self.$text, placeholder: "NoteFormView", errorMessage: "", hintText: AttributedString("Hint Text"), isCharCountEnabled: true, allowsBeyondLimit: false)
                ...
                .waTextInput(self.$text, menus: WAMenu.defaultMenus, menuHandler: { menu, value in
                    await self.fetchData(for: menu, value: value)
                }, feedbackOptions: ["1", "2"], feedbackHandler: { state, values in
            ...
            TextInputField(text: self.$text2)
                ...
                .border(Color.black)
                .waTextInput(self.$text2, menus: WAMenu.defaultMenus, menuHandler: { menu, value in
                    await self.fetchData(for: menu, value: value)
                }, feedbackOptions: ["1", "2"], feedbackHandler: { state, values in
                    await self.submitFeedback(state: state, values: values)
                })
